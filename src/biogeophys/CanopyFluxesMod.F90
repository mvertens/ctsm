...
bioms:   do f = 1, fn
      ...
      ITERATION : do while (itlef <= itmax_canopy_fluxes .and. fn > 0)
         ...
         do f = 1, fn
            ...
            !            'CanopyFluxes DEBUG: nstep,p,liqcan_patch,snocan_patch = ',&
            !            get_nstep(),p,liqcan(p),snocan(p)
            !    end if
            ! end if
            ...
            if (close_to(lon, lat, 0.26718750000000E+03_r8, 0.29033187709747E+02_r8)) then
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,elai(p)              = ',get_nstep(),p,elai(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,esai(p)              = ',get_nstep(),p,esai(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,t_veg_before         = ',get_nstep(),p,t_veg(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,frac_rad_abs_by_stem = ',get_nstep(),p,frac_rad_abs_by_stem(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,sabv(p)              = ',get_nstep(),p,sabv(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,air(p)               = ',get_nstep(),p,air(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,bir(p)               = ',get_nstep(),p,bir(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,cir(p)               = ',get_nstep(),p,cir(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,lw_grnd              = ',get_nstep(),p,lw_grnd
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,efe(p)               = ',get_nstep(),p,efe(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,lw_leaf(p)           = ',get_nstep(),p,lw_leaf(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,lw_stem(p)           = ',get_nstep(),p,lw_stem(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,cp_leaf(p)           = ',get_nstep(),p,cp_leaf(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,tl_ini(p)            = ',get_nstep(),p,tl_ini(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,lw_stem(p)           = ',get_nstep(),p,lw_stem(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,sa_internal(p)       = ',get_nstep(),p,sa_internal(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,emv(p)               = ',get_nstep(),p,emv(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,wtga(p)              = ',get_nstep(),p,wtga(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,wtgaq                = ',get_nstep(),p,wtgaq
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,qsatldT(p)           = ',get_nstep(),p,qsatldT(p)
               ! write(6,'(a,2x,2(i0,2x),d20.14,2x)') 'CanopyFluxes DEBUG: nstep,p,dt_veg(p)            = ',get_nstep(),p,dt_veg(p)
            end if

            t_veg(p) = tlbef(p) + dt_veg(p)

            ! if (close_to(lon, lat, 0.26718750000000E+03_r8, 0.29033187709747E+02_r8)) then
            !    write(6,'(a,2x,2(i0,2x),3(d20.14,2x))') &
            !         'CanopyFluxes DEBUG: nstep,p,tlbef,dt_veg,t_veg_after = ',&
            !         get_nstep(),p,tlbef(p),dt_veg(p),t_veg(p)
            ! end if
      ...
      do f = 1, fn
         ...
         ! if (close_to(lon, lat, 0.26718750000000E+03_r8, 0.29033187709747E+02_r8)) then
         !    write(6,'(a,2x,2(i0,2x),2(d20.14,2x))') &
         !         'CanopyFluxes DEBUG: nstep,p,t_veg,tfrz = ',&
         !         get_nstep(),p,t_veg(p),tfrz
         !    write(6,'(a,2x,2(i0,2x),2(d20.14,2x))') &
         !         'CanopyFluxes DEBUG: nstep,p,qflx_evap_veg,qflx_tran_veg = ', &
         !         get_nstep(),p,qflx_evap_veg(p), qflx_tran_veg(p)
         ! end if
